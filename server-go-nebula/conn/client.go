...
func Connect(address string, port int, username string, password string) (nsid string, err error) {
	nsid, err = NewConnection(address, port, username, password)
	if err != nil {
		return "", err
	}
	return nsid, err
...
func Disconnect(nsid string) {
	DisconnectFromPool(nsid)
}

// nsid string,
func Execute(gql string) (result ExecuteResult, err error) {
	var nsid string = Nsid
	result = ExecuteResult{
		Headers: make([]string, 0),
		Tables:  make([]map[string]Any, 0),
	}
	connection, err := GetConnection(nsid)
	if err != nil {
		return result, err
	}

	responseChannel := make(chan ChannelResponse)
	connection.RequestChannel <- ChannelRequest{
		Gql:             gql,
